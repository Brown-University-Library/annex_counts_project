...
- passenger env-vars loading under python3.x is enabled via the `SenEnv` entry in the project's httpd/passenger.conf section.
  - usage: `SetEnv ANX_ACC__SETTINGS_PATH /path/to/ANX_ACC__env_settings.sh`
...
For activating the virtual-environment manually, don't source the settings file directly. Instead, add to `project_env/bin/activate`:
  export ANX_ACC__SETTINGS_PATH="/path/to/ANX_ACC__env_settings.sh"
  source $ANX_ACC__SETTINGS_PATH
...
ENV_SETTINGS_FILE = os.environ['ANX_ACC__SETTINGS_PATH']  # set in `httpd/passenger.conf`, and `env/bin/activate`
